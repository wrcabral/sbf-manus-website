...
- Home única com seções (âncoras) + páginas: `/metodo-real`, `/rota-tributaria`, `/blog` (248 posts em `content/posts.json`, gerados no build; série da Reforma Tributária publicada em 23/09/2026). Leads: todos os formulários enviam ao mesmo Apps Script, que roda na conta `wrcabral82@gmail.com` (Wanderson), não na do Bruno, `/politica-de-privacidade`, `/obrigado`.
